...
function makeMap(mapNode) {
    ...
    navigator.geolocation.getCurrentPosition(function (data) {
        ...
        var dynamicOptions = { center: { lat: lat, lng: lng }, zoom: 12 };
        map = new google.maps.Map(mapNode, dynamicOptions);
        loadPlaces(map, lat, lng);
    }, function (err) {
        console.error(err);
        map = new google.maps.Map(mapNode, fallbackOptions);
        loadPlaces(map);
    });
...
function typeAhead(search) {
    ...
    searchInput.addEventListener('input', function () {
        ...
        if (!this.value) {
            ...
            _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
                return searchResults.innerHTML = _dompurify2.default.sanitize(searchResultsHTML(res.data, _this.value));
            }).catch(function (err) {
                console.error(err);
            });
        }
    ...
    searchInput.addEventListener('keyup', function (e) {
        ...
        var results = search.querySelectorAll('.search__result');
        var next = void 0;
        switch (e.keyCode) {
            case 40:
                next = current ? current.nextElementSibling || results[0] : results[0];
                break;
            case 38:
                next = current ? current.previousElementSibling || results[results.length - 1] : results[results.length - 1];
                break;
            case 13:
